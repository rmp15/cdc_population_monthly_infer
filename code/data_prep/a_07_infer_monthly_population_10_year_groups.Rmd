---
title: "Infer monthly population (10-year age groups)"
...
for(year_selected in c(min(dat_complete$year):end_year_cdc_single_race)){
  ...
  if(year_selected>=2021){
    filename_out = paste0(population.10year.processed.folder,
                      'vintage_',vintage_single_race,'/pop_monthly_10_year_age_groups_', year_selected,'.csv')
  }
...
new_population_data.10year = paste0(new_population_data,'10_year_age_groups/')

for(year_selected in c(min(dat_complete$year):end_year_cdc_single_race)){
  dat_year = dat_complete %>% dplyr::filter(year==year_selected)
  if(year_selected<1990){
    filename_out = paste0(new_population_data.10year,
                      'pre_1990/pop_monthly_10_year_age_groups_', year_selected,'.csv')  
  }
  if(year_selected>=1990&year_selected<=2020){
    filename_out = paste0(new_population_data.10year,
